...
const getAlertMessage = (type, price) => {
...
const checkAlerts = async () => {
    ...
    isChecking = true;
    try {
        const { rows: systemRows } = await pool_1.pool.query(`SELECT global_alert_enabled FROM system_settings LIMIT 1`);
        if (!systemRows.length || !systemRows[0].global_alert_enabled)
            return;
        const baseKlines = await (0, price_service_1.getKlines)({ interval: "1m", limit: 30 });
        if (baseKlines.length < 15)
            return;
        const currentPrice = baseKlines[baseKlines.length - 1].close;
        const { rows: alerts } = await pool_1.pool.query(`SELECT * FROM alerts WHERE is_active = true`);
        if (!alerts.length)
            return;
        const { rows: users } = await pool_1.pool.query(`SELECT id, email FROM users`);
        ...
        for (const alert of alerts) {
            let triggered = false;
            if (alert.type === "PRICE_UP" && alert.target_price !== null && currentPrice >= alert.target_price)
                triggered = true;
            if (alert.type === "PRICE_DOWN" && alert.target_price !== null && currentPrice <= alert.target_price)
                triggered = true;
            if (alert.type === "VOLATILITY" && alert.time_window && alert.percentage && alert.time_window <= baseKlines.length) {
                ...
                if (Math.abs(change) >= alert.percentage)
                    triggered = true;
            }
            ...
            if (alert.type === "PREDICT_UP" && prediction?.direction === "UP" && prediction.confidence >= (alert.confidence_threshold ?? 0))
                triggered = true;
            if (alert.type === "PREDICT_DOWN" && prediction?.direction === "DOWN" && prediction.confidence >= (alert.confidence_threshold ?? 0))
                triggered = true;
            if (triggered) {
                const displayMessage = getAlertMessage(alert.type, currentPrice);
                // 1. Save Notification
                const result = await pool_1.pool.query(`INSERT INTO notifications (user_id, alert_id, message) VALUES ($1, $2, $3) RETURNING *`, [alert.user_id, alert.id, displayMessage]);
                // 2. Realtime Socket.io
                server_1.io.to(`user_${alert.user_id}`).emit("new_notification", result.rows[0]);
                ...
                if (email) {
                    ...
                    if (!lastSent || now - lastSent > EMAIL_COOLDOWN_MS) {
                        try {
                            ...
                            await (0, mail_service_1.sendAlertEmail)(email, displayMessage);

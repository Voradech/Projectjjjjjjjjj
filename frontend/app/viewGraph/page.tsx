...
// จำนวนแท่งเริ่มต้นที่ดึง (ปรับได้)
const RANGE_LIMIT: Record<RangeKey, number> = {
  "1D": 600, // ~10 ชม. ถ้าอยากเต็มวันละเอียดจริงๆ เพิ่มได้ (แต่หนัก)
  "7D": 7 * 24,
  "1M": 30,
  "1Y": 365,
};
...
export default function ViewGraphPage() {
  ...
  const loadAllAndShowOnChart = async () => {
    ...
    try {
      ...
      // ถ้ายังไม่มีข้อมูล ให้ดึงรอบแรกก่อน (ล่าสุด 1000)
      if (all.length === 0) {
      ...
      const MAX_ROUNDS = 30; // 30*1000 = 30,000 แท่ง (เยอะมาก)
      const SLEEP_MS = 250; // หน่วงกันโดน rate limit
  ...
  return (
    <div className="w-full p-4 space-y-3">
      <div className="flex items-center gap-2">
        {(["1D", "7D", "1M", "1Y"] as RangeKey[]).map((k) => (
          <button
            key={k}
            onClick={() => setRange(k)}
            className={`px-3 py-1 rounded-lg border ${range === k ? "font-semibold" : ""}`}
          >
            {k}
          </button>
        ))}
      ...
      <div ref={chartRef} className="w-full border rounded-xl p-2" />

      <div className="text-xs opacity-70">
        Interval: <b>{interval}</b> | Loaded: <b>{candles.length}</b> | Realtime: <b>ON</b>
      </div>
    </div>
  );
}

...
export default function PredictView() {
  ...
  useEffect(() => {
    ...
    // Run initial prediction on mount — safe: runPredict defined above
    runPredict();
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white/[0.05] backdrop-blur-xl border border-white/10 rounded-2xl p-6 relative overflow-hidden group hover:border-emerald-500/30 transition-all">
              ...
              <div className="text-xl font-bold text-white capitalize flex items-center gap-2">
                {selectedModel ? selectedModel.replace("_", " ") : "Best Model"}
                {confidence && (
              ...
              <div className="text-sm text-indigo-300 mt-1">
                ทำนายล่วงหน้า {horizon} วัน ({horizon === 1 ? "1 วัน" : horizon === 7 ? "1 สัปดาห์" : "2 สัปดาห์"})
              </div>
            </div>
          </div>
        )}
